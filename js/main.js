...
async function processarArquivoZip(file) {
    try {
        ...
        if (!file.name.toLowerCase().endsWith('.zip')) {
            throw new Error('O arquivo deve ser um ZIP');
        }
        ...
        for (const [key, pattern] of Object.entries(filePatterns)) {
            ...
            if (matchingFiles.length > 0) {
                ...
                throw new Error(`Arquivo com padrão ${pattern} não encontrado no ZIP. Verifique se o arquivo contém os arquivos necessários.`);
            }
        }
